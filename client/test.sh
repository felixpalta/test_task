...
function print_usage {
  echo "Usage: $0 pipe_to_server pipe_from_server" >&2
  exit 1
}
...
function color_test {
  for color in "${COLORS[@]}"
  do
    COLOR_RESULT=`./client.sh $TO_SERVER $FROM_SERVER --set color $color`
    ...
    COLOR_RESULT=`./client.sh $TO_SERVER $FROM_SERVER --get color`
...
function state_test {
  for state in "${STATES[@]}"
  do
    STATE_RESULT=`./client.sh $TO_SERVER $FROM_SERVER --set state $state`
    ...
    STATE_RESULT=`./client.sh $TO_SERVER $FROM_SERVER --get state`
...
function rate_test {
  for rate in "${RATES[@]}"
  do
    RATE_RESULT=`./client.sh $TO_SERVER $FROM_SERVER --set rate $rate`
    ...
    RATE_RESULT=`./client.sh $TO_SERVER $FROM_SERVER --get rate`
